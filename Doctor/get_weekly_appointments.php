...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $index = ($weekday + 5) % 7; // Convert to 0 (Mon) - 6 (Sun)
        $weeklyAppointments[$index] = (int)$row['count'];
    }

    echo json_encode($weeklyAppointments);
} catch (Exception $e) {
    error_log("Error fetching weekly appointments: " . $e->getMessage());
    echo json_encode([0, 0, 0, 0, 0, 0, 0]);
}
?>